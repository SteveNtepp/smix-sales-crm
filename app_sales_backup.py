...
def wa_link(phone: str, message: str) -> str:
    """Build WhatsApp URL — UTF-8 quote on string (not bytes) keeps emoji intact."""
...
def page_dashboard(user: dict):
    ...
    top = db.get_weekly_top_closer()
    if top and (is_admin or top == user["username"]):
        ...
        <div class="top-closer-banner">
          ...
          <div class="top-closer-text">
            <strong>Top Closer de la semaine : {names.get(top, top.title())} 🔥</strong>
            <span>Meilleur taux de conversion sur les 7 derniers jours</span>
          </div>
        </div>""", unsafe_allow_html=True)
        if not is_admin and top == user["username"]:
            st.balloons()
...
def page_scripts(user: dict):
    ...
    lead = db.get_lead(opts[sel]); cfg = db.get_config(); scripts = db.get_scripts()
    for dtab, day in zip(st.tabs(["🟡 J+1","🔵 J+3","🔴 J+5","⚫ J+7"]), ["J1","J3","J5","J7"]):
        with dtab:
...
def page_cockpit_admin(user: dict):
    ...
    with t7:
        ...
        stats = db.get_team_stats(); top_c = db.get_weekly_top_closer()
        if stats.empty:
